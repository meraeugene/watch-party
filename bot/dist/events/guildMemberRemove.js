...
async function guildMemberRemove(member) {
    if (!member.guild || !member.user || !member.guild.id)
        return;
    ...
    if (!ticket)
        return;
    const normalizedTextChannelName = `💞-${ticket.host_name}-${ticket.guest_name}`
        .toLowerCase()
        .replace(/\s+/g, "-");
    const voiceChannelName = `🎬 - ${ticket.movie_title}`;
    const textChannel = member.guild.channels.cache.find((c) => c.name === normalizedTextChannelName);
    const voiceChannel = member.guild.channels.cache.find((c) => c.name === voiceChannelName);
    if (textChannel)
        await textChannel.delete().catch(console.error);
    if (voiceChannel)
        await voiceChannel.delete().catch(console.error);
    await client_supabase_1.supabase
        .from("watch_party_tickets")
        .update({ status: "left" })
        .eq("id", ticket.id);
