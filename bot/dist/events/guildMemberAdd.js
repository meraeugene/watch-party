...
async function guildMemberAdd(member, client) {
    ...
    if (error || !tickets?.length)
        return console.log("No pending tickets");
    const usedTicket = tickets[0];
    if (!usedTicket) {
        console.log(`👤 No pending ticket found for ${member.user.id}`);
        return;
    }
    const { error: updateError } = await client_supabase_1.supabase
        .from("watch_party_tickets")
        .update({
    ...
    if (hostMember) {
        hostMember
            .send(`🎉 Your guest **${usedTicket.guest_name}** has joined your private movie night!\n\n` +
    ...
    const loungeChannel = member.guild.channels.cache.find((c) => c.name === "watch-party-lounge" && c.type === discord_js_1.ChannelType.GuildText);
    loungeChannel?.send(`💘 A new **private movie date** just kicked off! \n\n` +
        `Movie: **${usedTicket.movie_title}**\n` +
        `Date: **${usedTicket.date}** \n` +
        `Time: **${usedTicket.time}**\n` +
        `Runtime: **${usedTicket.movie_duration} minutes**\n` +
    ...
    await textChannel.send({
        content: `👋 Welcome <@${member.id}> and <@${usedTicket.host_discord_id}>! This is your private watch party channel for **${usedTicket.movie_title}**.\n\n` +
            `**Date:** ${usedTicket.date}\n**Time:** ${usedTicket.time}\n` +
            `**Runtime:** ${usedTicket.movie_duration} minutes\n\n` +
    ...
    member
        .send(`🎉 You've been invited by **${usedTicket.host_name}** to a private movie night!\n\n` +
        `**Movie:** ${usedTicket.movie_title}\n` +
        `**Date:** ${usedTicket.date}\n` +
        `**Time:** ${usedTicket.time}\n` +
        `**Runtime:** ${usedTicket.movie_duration} minutes\n\n` +
    ...
    if (!movieDateTime.isValid) {
        return console.error("❌ Invalid movie datetime:", usedTicket.date, usedTicket.time);
    }
    ...
    console.log(`⏰ Scheduling reminder in ${delay / 1000}s for ${member.user.tag}`);
    if (delay > 0) {
        setTimeout(async () => {
            const chan = member.guild.channels.cache.get(textChannel.id);
            if (chan) {
                chan.send(`⏰ <@${member.id}> Reminder: Your movie **${usedTicket.movie_title}** starts in 1 hour! 🍿`);
            }
            member
                .send(`⏰ Reminder: Your movie **${usedTicket.movie_title}** starts in 1 hour!\nJoin your private channel: #${textChannel.name}`)
                .catch(() => {
